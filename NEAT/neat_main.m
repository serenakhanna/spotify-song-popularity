...
function neat_main(genre)
clearvars -except genre
tic;
%list of parameters
  %dataset parameter
  ...
  %parameters initial population
    population_size=100;
    number_input_nodes=n-1;
    number_output_nodes=1;
    
    vector_connected_input_nodes=[]; %vector of initially connected input nodes out of complete number of input nodes 
...
%     vector_connected_input_nodes=1:number_input_nodes;
  ...
  %reproduction parameters
    ...
    initial.number_for_kill=0.04*population_size; % the above percentage for eliminating individuals will only be used in species which have more individuals than min_number_for_kill 
    ...
    initial.number_copy=0.04*population_size; % species which have equal or greater than number_copy individuals will have their best individual copied unchanged into the next generation
    ...
    fileID = fopen("out_"+genre+".txt",'w');
    fprintf(fileID,'Max Generation: %d ,Population Size: %d \n',maxgeneration,population_size);
    fclose(fileID);
...
while generation<maxgeneration & flag_solution==0
   ...
   if save_flag==1 % Backup copies of current generation
      save("neatsave_"+genre,'population','generation','innovation_record','species_record')
   end
   ...
   fileID = fopen("times/out_"+genre+".txt",'a');
   fprintf(fileID,'%d,',generation);
   fclose(fileID);
   
   
   population=experiment(population,genre);
   ...
   ylabel('max fitness');
   subplot(2,2,4);
   plot(max_overall_fitness(2,:),(m-sqrt(max_overall_fitness(1,:)))/m);
   title("Error of the Fittest Individual");
   ylabel('error of max fit');
   sgtitle("Genre: "+genre+" Generation: "+num2str(generation)+" Population Size: "+num2str(population_size));
   drawnow;
